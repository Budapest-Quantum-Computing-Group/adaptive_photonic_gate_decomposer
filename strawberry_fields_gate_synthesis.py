# The following script is not our contribution and can be found at:
...
# on qumode ``q``. We add the :class:`~strawberryfields.utils.operation` decorator so that the layer can be used
...
target_ham = gates.cubic_phase_hamiltonian(cutoff+20)[:cutoff, :cutoff]
target_gate = gates.get_unitary(0.01, target_ham)
target_unitary = np.identity(cutoff, dtype=np.complex128)
target_unitary[:, :gate_cutoff] = target_gate[:, :gate_cutoff]
...
print(f"Process took {RUNTIME} seconds")
...
print(f"Average fidelity{np.abs(np.vdot(psiV, psiU))**2}")
